...
def decode_predictions(predictions: Tensor, anchors: Tensor, S: int) -> Tensor:
    ...
    decoded[..., :4] = xywh2xyxy(decoded[..., :4]).clamp(0, 1)
    decoded[..., 4:] = predictions[..., 4:]

    return decoded


def decode_target(target: Tensor, anchor_bboxes: Tensor, S: int) -> Tensor:
    decoded = decode_predictions(target, anchor_bboxes, S)
...
def postprocess_predictions(
    ...
    *,
    S: int = 13,
    imgsz: tuple[int, int] = (416, 416),
    num_classes: int = 20,
    conf_thresh: float = 0.2,
    nms_thresh: float = 0.4,
) -> list[Prediction]:
    decoded_preds = decode_predictions(predictions, anchors, S)
    decoded_preds = decoded_preds.reshape(decoded_preds.size(0), -1, 5 + num_classes)

    img_size = torch.tensor(imgsz, dtype=torch.float32).repeat(1, 2)
    filtered = list()

    for pred in decoded_preds:
        labels = pred[:, 5:].argmax(dim=1)
        xyxys = pred[:, :4] * img_size
